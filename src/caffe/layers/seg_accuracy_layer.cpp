...
void SegAccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  */
  ...
  top[0]->mutable_cpu_data()[0] = (Dtype)confusion_matrix_.avgDiceCoefficient(false);
  top[0]->mutable_cpu_data()[1] = (Dtype)confusion_matrix_.diceCoefficient(1);
  top[0]->mutable_cpu_data()[2] = (Dtype)confusion_matrix_.avgRecall(false);
