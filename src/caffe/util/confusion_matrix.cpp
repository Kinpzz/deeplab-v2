...
double ConfusionMatrix::avgRecall(const bool strict) const {
  ...
  /* 2017.08.28 kinpzz 
    ---- confusion mask ----
     ...
     row_sum(1) : X cross Y
     col_sum(1) : X + Y
     Dice coefficient: 2(X cross Y)/(X + Y)
  */
double ConfusionMatrix::avgDiceCoefficient(const bool strict) const {
  double totalDice = 0.0;
  int numClasses = 0;
  for (size_t i = 0; i < _matrix.size(); i++) {
    if (_matrix[i].size() > i) {
      const double unionSize = (double)rowSum(i) + (double)colSum(i);
      if (unionSize > 0.0) {
        totalDice += 2*(double)_matrix[i][i] / unionSize;
	      numClasses += 1;
      }
    }
  }
  ...
  if (strict && numClasses != (int)_matrix.size()) {
    LOG(FATAL) << "not all classes represented in avgDiceCoefficient()";
  }

  if (numClasses == 0) {
    return 0;
  } else {
    return totalDice / (double)numClasses;
  }
}
...
double ConfusionMatrix::diceCoefficient(int n) const {
  CHECK(_matrix.size() > (size_t)n);
  return (_matrix[n].size() > (size_t)n) ?
    (2*(double)_matrix[n][n]) / ((double)rowSum(n) + (double)colSum(n))  : 0.0;
}
